...
record Process(int processId, Site site) implements Runnable {
...
class CentralServer extends Site {
    // Singleton — only one central server allowed
    ...
    // Disallow manual instantiation of CentralServer
    private CentralServer(int siteId) {
        super(siteId);
    }

    // Enforce singleton by allowing only one instance of CentralServer
    static CentralServer getInstance(int siteId) {
        if (instance == null)
            instance = new CentralServer(siteId);
        return instance;
    }
    ...
    static void buildWaitForGraph() {
        ...
        Main.allProcesses.forEach(p -> waitForGraph.put(p.processId(), new LinkedHashSet<>()));

        // Construct WFG
        Main.allProcesses.forEach(p -> {
    ...
    static boolean detectDeadlock() {
        int n = Main.allProcesses.size();
...
class Main {
    ...
    static Set<Site> allSites = new LinkedHashSet<>();
    ...
    public static void main(String[] args) {
        ...
        CentralServer.buildWaitForGraph();

        // Display the generated wait-for graph
        CentralServer.printWaitForGraph();

        // A cycle in the wait-for graph indicates a deadlock
        if (CentralServer.detectDeadlock()) System.out.println("\nDeadlock detected.");
    ...
    static void inputSiteProcessResourceCount() {
        ...
        System.out.print("Which site should be the central server? (0 - " + (nSites - 1) + ") ");
        int centralServerId = sc.nextInt();
        if (centralServerId < 0 || centralServerId >= nSites) {
            System.out.println("Invalid central server id: " + centralServerId);
            return;
        }
        ...
        for (int i = 0; i < nSites; i++) {
            Site currentSite = centralServerId == i ? CentralServer.getInstance(i) : new Site(i);
            allSites.add(currentSite);
        ...
        System.out.println("Central server: " + centralServerId);
    }
